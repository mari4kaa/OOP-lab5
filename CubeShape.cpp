...
void CubeShape::Show(HDC hdc, BOOL PaintingNow, BOOL isSelected)
{
    ...
    SetAll(x1_proect, y1_proect, x2_proect, y2_proect);
    RectShape::Show(hdc, PaintingNow, isSelected);

    SetAll((x1 * 2) - x2, (y1 * 2) - y2, (x1_proect * 2) - x2_proect, (y1_proect * 2) - y2_proect);
    LineShape::Show(hdc, PaintingNow, isSelected);

    SetAll((x1 * 2) - x2, y2, (x1_proect * 2) - x2_proect, y2_proect);
    LineShape::Show(hdc, PaintingNow, isSelected);

    SetAll(x2, y2, x2_proect, y2_proect);
    LineShape::Show(hdc, PaintingNow, isSelected);

    SetAll(x2, (y1 * 2) - y2, x2_proect, (y1_proect * 2) - y2_proect);
    LineShape::Show(hdc, PaintingNow, isSelected);
